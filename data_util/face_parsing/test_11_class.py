...
def vis_parsing_maps(im, parsing_anno, stride, save_im=False, save_path='vis_results/parsing_map_on_im.jpg',
    ...
    for pi in range(1, num_of_class+1):
        ...
        if pi == 1:
            ...
            vis_parsing_anno_color[index[0], index[1], :] = np.array([173, 202, 25])
        elif pi == 2 or pi == 3:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([181, 199, 140])
        elif pi == 4 or pi == 5:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([225, 238, 160])
        elif pi == 7 or pi == 8:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([255, 0, 0])
        elif pi == 10:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([199, 237, 190])
        elif pi == 11:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([183, 213, 214])
        elif pi == 12 or pi == 13:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([116, 186, 209])
        elif pi == 14:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([172, 206, 230])
        elif pi == 16:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([158, 173, 236])
        elif pi == 17:
            index = np.where(vis_parsing_anno == pi)
            vis_parsing_anno_color[index[0], index[1], :] = np.array([108, 96, 244])
        else:
            pass
